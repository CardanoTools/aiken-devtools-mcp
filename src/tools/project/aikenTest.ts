...
const inputSchema = z
  .object({
    ...
    extraArgs: z
      .array(z.string())
      .optional()
      .describe("Additional args passed to `aiken test` (advanced)."),
...
export function registerAikenTestTool(server: McpServer): void {
  server.registerTool(
    "aiken_test",
    {
      title: "Aiken: test",
      description: "Runs `aiken test` to execute the project's test suite.",
      inputSchema,
      outputSchema,
      annotations: {
        readOnlyHint: true,
