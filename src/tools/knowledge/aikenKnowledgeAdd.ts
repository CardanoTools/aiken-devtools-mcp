...
import { addCustomSource, ensureIndexExportsForCategory, type Category } from "../../knowledge/ingestion/customManager.js";
import { runGit } from "../../git/runGit.js";
import { findSpecById } from "../../knowledge/core/registry.js";
import { ALL_KNOWLEDGE_SOURCES } from "../../knowledge/index.js";
import { KnowledgeSourceSpec } from "../../knowledge/core/types.js";
  ...
  server.registerTool(
    ...
    async ({ remoteUrl, id, category: categoryIn, subPath, description, defaultRef, folderName, commit, runSync }) => {
      ...
      if (!added.ok) {
        ...
        return { isError: true, content: [{ type: "text", text: `Failed to write custom file: ${(added as { ok: false; reason: string }).reason}` }] };
      }

      const ensured = await ensureIndexExportsForCategory(category);
      if (!ensured.ok) {
        return { isError: true, content: [{ type: "text", text: `Added source but failed to update index.ts: ${(ensured as { ok: false; reason: string }).reason}` }], structuredContent: { id: spec.id, path: added.path, committed: false } };
      }
      ...
      if (commit ?? true) {
        ...
        if (!gitAdd.ok) {
          return { isError: true, content: [{ type: "text", text: `Added source but git add failed: ${(gitAdd as { ok: false; error: string }).error}` }], structuredContent: { id: spec.id, path: added.path, committed: false } };
        }
