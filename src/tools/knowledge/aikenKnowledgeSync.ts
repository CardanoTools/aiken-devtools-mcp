...
import { KnowledgeSource, KnowledgeSourceSpec } from "../../knowledge/core/types.js";
import { ALL_KNOWLEDGE_SOURCES } from "../../knowledge/index.js";
import { getCacheBaseDir, resolveSourceDirPath, resolveRepoBaseDirPath } from "../../knowledge/core/utils.js";
...
async function syncOne(source: KnowledgeSource, refOverride: string | undefined, timeoutMs?: number) {
  ...
  if (!isRepoDir) {
    ...
    if (!clone.ok) {
      return {
        ...
        ok: false,
        error: (clone as { ok: false; error: string }).error
      };
    }
  ...
  if (!fetch.ok) {
    return {
      ...
      description: spec.description,
      ok: false,
      error: (fetch as { ok: false; error: string }).error
    };
  }
  if (fetch.exitCode !== 0) {
    return {
  ...
  if (!checkout.ok) {
    return {
      ...
      description: spec.description,
      ok: false,
      error: (checkout as { ok: false; error: string }).error
    };
  }
