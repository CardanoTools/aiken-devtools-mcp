...
import { runAiken, resolveWorkspacePath, type RunAikenResult } from "../../aiken/runAiken.js";
...
import { generateEvolutionSdkSnippet, generateLucidEvolutionSnippet } from "../../codegen/snippets.js";
...
export function registerAikenBlueprintIntegrationBundleAllTool(server: McpServer): void {
  server.registerTool(
    ...
    async ({
      ...
      if (!blueprint.ok) return toBlueprintToolError((blueprint as { ok: false; error: string }).error);
      ...
      for (const entry of validators) {
        ...
        result.errors = result.errors ?? [];
        if (!hashStep.ok) result.errors.push(`hash: ${(hashStep as { ok: false; error: string }).error}`);
        if (!addressStep.ok) result.errors.push(`address: ${(addressStep as { ok: false; error: string }).error}`);
        if (!policyStep.ok) result.errors.push(`policy: ${(policyStep as { ok: false; error: string }).error}`);
        if (result.steps.convert && !result.steps.convert.ok) {
          result.errors.push(`convert: ${(result.steps.convert as { ok: false; error: string }).error}`);
        }
