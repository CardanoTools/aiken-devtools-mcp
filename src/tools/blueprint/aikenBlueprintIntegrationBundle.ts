...
import { resolveWorkspacePath, runAiken, type RunAikenResult } from "../../aiken/runAiken.js";
...
export async function computeIntegrationBundle(
  ...
  const errors: string[] = [];
  if (!hashStep.ok) errors.push(`hash: ${(hashStep as { ok: false; error: string }).error}`);
  if (!addressStep.ok) errors.push(`address: ${(addressStep as { ok: false; error: string }).error}`);
  if (!policyStep.ok) errors.push(`policy: ${(policyStep as { ok: false; error: string }).error}`);
  if (structuredContent.steps.convert && !structuredContent.steps.convert.ok) {
    errors.push(`convert: ${(structuredContent.steps.convert as { ok: false; error: string }).error}`);
  }
