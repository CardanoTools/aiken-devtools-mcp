...
export function registerAikenBlueprintPolicyTool(server: McpServer): void {
  server.registerTool(
    "aiken_blueprint_policy",
    {
      title: "Aiken: blueprint policy",
      description:
        "Runs `aiken blueprint policy` to compute a minting policy ID from the blueprint (default: plutus.json).",
      inputSchema,
      outputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    async ({ projectDir, blueprintPath, module, validator, timeoutMs }) => {
      ...
      const args: string[] = ["blueprint", "policy"];
